...
@section('content')
    ...
    <section
        class="relative min-h-[380px] md:min-h-[420px] lg:min-h-[460px] flex items-center bg-gradient-to-br from-[#f8f4ed] via-[#f5f0fa] to-[#f0f9ff] overflow-hidden">

        <!-- Soft Pastel Background Pattern -->
        <div
            class="absolute inset-0 bg-[radial-gradient(#f4a261_0.8px,transparent_1px)] [background-size:40px_40px] opacity-10">
        </div>
        ...
        <div class="relative max-w-7xl mx-auto px-6 w-full z-10">
            <div class="max-w-2xl">
                ...
                <div class="mt-10">
                    <a href="#products"
                        class="inline-flex items-center gap-3 bg-gradient-to-r from-[#f4a261] to-[#e07a5f] text-white px-10 py-4 rounded-2xl font-semibold text-lg hover:shadow-xl transition-all">
                        Explore Collection
            ...
            <div class="lg:w-80 flex-shrink-0">
                <div class="lg:hidden mb-3 md:mb-6">
                    ...
                    <div id="categoryDrawer"
                        class="lg:static fixed inset-y-0 left-0  w-80 bg-white shadow-2xl lg:shadow-none border-r border-gray-200 transform -translate-x-full lg:translate-x-0 transition-transform duration-300 overflow-y-auto"
                        style="z-index:9999;">
                        ...
                        <div class="lg:hidden flex items-center justify-between p-5 border-b bg-white sticky top-0 z-10">
                            ...
                            <button onclick="toggleCategoryDrawer()"
                                class="text-3xl text-gray-500 hover:text-gray-700">✕</button>
                        </div>
                        ...
                        <div class="sidebar-card p-6 lg:p-0 ">
                            ...
                            @foreach($categories as $cat)
                                ...
                                <div
                                    class="sub-category pl-6 space-y-2 mt-1 {{ $parentCategory && $parentCategory->id == $cat->id ? '' : 'hidden' }}">
                                    @foreach($cat->children as $sub)
                                        <a href="{{ route('products', ['subcategory' => $sub->slug]) }}"
                                            class="block py-2 hover:text-[#f4a261] {{ request('subcategory') == $sub->slug ? 'text-[#f4a261] font-semibold' : '' }}">
                                            {{ $sub->name }}
                                        </a>
                                    @endforeach
                                </div>
                            ...
                            <form method="GET" class="mt-12">
                                ...
                                <div class="mb-8">
                                    ...
                                    <div class="flex flex-wrap gap-2">
                                        @foreach($customizations as $custom)
                                            <label>
                                                <input type="checkbox" name="customization[]" value="{{ $custom->id }}"
                                                    class="hidden" {{ in_array($custom->id, request('customization', [])) ? 'checked' : '' }}>
                                ...
                                <div class="mb-8">
                                    ...
                                    @foreach($brands as $brand)
                                        <label class="flex items-center gap-2 mb-2">
                                            <input type="checkbox" name="brand[]" value="{{ $brand->id }}" accept="" {{ in_array($brand->id, request('brand', [])) ? 'checked' : '' }}>
                                            {{ $brand->name }}
                                        </label>
                                    @endforeach
                                </div>
                                ...
                                <button type="submit"
                                    class="w-full mt-8 bg-gradient-to-r from-[#f4a261] to-[#e07a5f] text-white py-3.5 rounded-2xl font-medium">
                                    Apply Filters
                                </button>
                            </form>
                        </div>
                    ...
                    <form method="GET" class="mt-12">
                        ...
                        <div class="mb-8">
                            ...
                            <div class="flex flex-wrap gap-2">
                                @foreach($occasions as $occ)
                                    <label>
                                        <input type="checkbox" name="occasion[]" value="{{ $occ->slug }}" class="hidden" {{ in_array($occ->slug, request('occasion', [])) ? 'checked' : '' }}>

                                        <span onclick="this.classList.toggle('active')" class="filter-chip 
                        ...
                        <div class="mb-8">
                            ...
                            @foreach($brands as $brand)
                                <label class="flex items-center gap-2 mb-2">
                                    <input type="checkbox" name="brand[]" value="{{ $brand->id }}" {{ in_array($brand->id, request('brand', [])) ? 'checked' : '' }}>
                                    {{ $brand->name }}
                                </label>
                            @endforeach
                        </div>
                        ...
                        <div class="mb-8">
                            ...
                            <label class="flex items-center gap-2">
                                <input type="checkbox" name="ready_to_ship" value="1" {{ request('ready_to_ship') ? 'checked' : '' }}>Ready to Ship
                            </label>
                        </div>
    ...
    <div id="drawerOverlay" onclick="toggleCategoryDrawer()" class="lg:hidden fixed inset-0 bg-black/60 z-40 hidden"></div>
